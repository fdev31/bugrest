...
if not os.path.exists(JSCODEFILE):
    ...
    import betterRest
    shutil.copy(
        os.path.join(os.path.dirname(betterRest.__file__), 'style.css'),
        STYLEFILE)
    shutil.copy(
        os.path.join(os.path.dirname(betterRest.__file__), 'code.js'),
        JSCODEFILE)
...
SHORT_DISPLAY = False
USE_ICONS = {'ACTIVE': ' ', 'STOPPED': ' ', 'LOOKRIGHT': '»', 'SEPARATOR': ' ❭ ', 'INACTIVE': '  '}
PLUGINS = []
...
class Bug:
    ...
    def string_as_list(self, i):
        ...
        return "%s%s%s #%d%s" % (
                styled(ICONS['ACTIVE'], 'bold') if self.started else (ICONS['STOPPED'] if self['timer'] != '' else ICONS['INACTIVE']),
...
class FileHandler:
    ...
    def load(self, done=False):
        ...
        if os.path.exists(fname):
            ...
            indices = [i for i, l in enumerate(text_lines) if l.startswith('------') and (i and not text_lines[i-1].strip())]
...
def cmd_list(handler):
    ...
    if max_bugid == 0 and len(handler) == 0 and verbose:
        ...
        if verbose:
            print("%s Fixed %s bugs%s%s active%slatest is #%d"%(
                ICONS['LOOKRIGHT'],
...
def cmd_get(handler, nr, name=None, *args):
    ...
    for bug in sorted(bugs, key=lambda o: (o[name], o.uid)):
        print("%s: %s # %s %s" % (styled(name, YELLOW), styled(bug[name], 'bold'), bug.title, styled("#%s"%bug.uid, TEAL)))
    ...
    for plugin in PLUGINS:
        try:
            ...
            if not entry_name in g and default_conf:
                print("%s is not configured, consider adding to your %s similar snippet:\n%s = %s"%(plugin, CFGFILE, entry_name, pprint.pformat(default_conf)))
            else:
                try:
                    plug_mod.Bug = Bug
                    ...
                    if conflicts:
                        for conflict in conflicts:
                            print('WARNING! "%s" is provided by %s and %s plugins, enable only one!'%(conflict[4:], plugin, commands[conflict].__code__.co_filename.rsplit(os.path.sep, 2)[1]))
