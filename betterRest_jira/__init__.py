...
class CFG:
    ...
    def asDict(kls):
        return dict(
            ...
            if not (k[0] == "_" or isinstance(v, classmethod))
        )
    ...
    for bugid in bugids:
        ...
        if jbug.fields.description:
            all_text = [
                ...
                % "\n".join(
                    "        " + l
                    for l in strip_tags(jbug.fields.description).split("\n")
                ),
                "\n\n.. raw:: html\n\n%s\n"
                % "\n".join(
                    "        " + l for l in jbug.fields.description.split("\n")
                ),
                "",
            ]
        else:
...
def get_jira_object():
    ...
    if _jira_instance is None:
        ...
        _jira_instance = JIRA(
            options,
            basic_auth=(conf.get("defaults", "user"), conf.get("defaults", "password")),
        )
